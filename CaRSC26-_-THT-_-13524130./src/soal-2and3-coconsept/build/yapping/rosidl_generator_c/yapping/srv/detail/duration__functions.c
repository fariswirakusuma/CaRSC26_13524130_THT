// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from yapping:srv/Duration.idl
// generated code does not contain a copyright notice
#include "yapping/srv/detail/duration__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `start_time`
// Member `end_time`
#include "rosidl_runtime_c/string_functions.h"

bool
yapping__srv__Duration_Request__init(yapping__srv__Duration_Request * msg)
{
  if (!msg) {
    return false;
  }
  // start_time
  if (!rosidl_runtime_c__String__init(&msg->start_time)) {
    yapping__srv__Duration_Request__fini(msg);
    return false;
  }
  // end_time
  if (!rosidl_runtime_c__String__init(&msg->end_time)) {
    yapping__srv__Duration_Request__fini(msg);
    return false;
  }
  return true;
}

void
yapping__srv__Duration_Request__fini(yapping__srv__Duration_Request * msg)
{
  if (!msg) {
    return;
  }
  // start_time
  rosidl_runtime_c__String__fini(&msg->start_time);
  // end_time
  rosidl_runtime_c__String__fini(&msg->end_time);
}

bool
yapping__srv__Duration_Request__are_equal(const yapping__srv__Duration_Request * lhs, const yapping__srv__Duration_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // start_time
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->start_time), &(rhs->start_time)))
  {
    return false;
  }
  // end_time
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->end_time), &(rhs->end_time)))
  {
    return false;
  }
  return true;
}

bool
yapping__srv__Duration_Request__copy(
  const yapping__srv__Duration_Request * input,
  yapping__srv__Duration_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // start_time
  if (!rosidl_runtime_c__String__copy(
      &(input->start_time), &(output->start_time)))
  {
    return false;
  }
  // end_time
  if (!rosidl_runtime_c__String__copy(
      &(input->end_time), &(output->end_time)))
  {
    return false;
  }
  return true;
}

yapping__srv__Duration_Request *
yapping__srv__Duration_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  yapping__srv__Duration_Request * msg = (yapping__srv__Duration_Request *)allocator.allocate(sizeof(yapping__srv__Duration_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(yapping__srv__Duration_Request));
  bool success = yapping__srv__Duration_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
yapping__srv__Duration_Request__destroy(yapping__srv__Duration_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    yapping__srv__Duration_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
yapping__srv__Duration_Request__Sequence__init(yapping__srv__Duration_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  yapping__srv__Duration_Request * data = NULL;

  if (size) {
    data = (yapping__srv__Duration_Request *)allocator.zero_allocate(size, sizeof(yapping__srv__Duration_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = yapping__srv__Duration_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        yapping__srv__Duration_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
yapping__srv__Duration_Request__Sequence__fini(yapping__srv__Duration_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      yapping__srv__Duration_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

yapping__srv__Duration_Request__Sequence *
yapping__srv__Duration_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  yapping__srv__Duration_Request__Sequence * array = (yapping__srv__Duration_Request__Sequence *)allocator.allocate(sizeof(yapping__srv__Duration_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = yapping__srv__Duration_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
yapping__srv__Duration_Request__Sequence__destroy(yapping__srv__Duration_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    yapping__srv__Duration_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
yapping__srv__Duration_Request__Sequence__are_equal(const yapping__srv__Duration_Request__Sequence * lhs, const yapping__srv__Duration_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!yapping__srv__Duration_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
yapping__srv__Duration_Request__Sequence__copy(
  const yapping__srv__Duration_Request__Sequence * input,
  yapping__srv__Duration_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(yapping__srv__Duration_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    yapping__srv__Duration_Request * data =
      (yapping__srv__Duration_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yapping__srv__Duration_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          yapping__srv__Duration_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!yapping__srv__Duration_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `iso_duration`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
yapping__srv__Duration_Response__init(yapping__srv__Duration_Response * msg)
{
  if (!msg) {
    return false;
  }
  // iso_duration
  if (!rosidl_runtime_c__String__init(&msg->iso_duration)) {
    yapping__srv__Duration_Response__fini(msg);
    return false;
  }
  return true;
}

void
yapping__srv__Duration_Response__fini(yapping__srv__Duration_Response * msg)
{
  if (!msg) {
    return;
  }
  // iso_duration
  rosidl_runtime_c__String__fini(&msg->iso_duration);
}

bool
yapping__srv__Duration_Response__are_equal(const yapping__srv__Duration_Response * lhs, const yapping__srv__Duration_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // iso_duration
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->iso_duration), &(rhs->iso_duration)))
  {
    return false;
  }
  return true;
}

bool
yapping__srv__Duration_Response__copy(
  const yapping__srv__Duration_Response * input,
  yapping__srv__Duration_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // iso_duration
  if (!rosidl_runtime_c__String__copy(
      &(input->iso_duration), &(output->iso_duration)))
  {
    return false;
  }
  return true;
}

yapping__srv__Duration_Response *
yapping__srv__Duration_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  yapping__srv__Duration_Response * msg = (yapping__srv__Duration_Response *)allocator.allocate(sizeof(yapping__srv__Duration_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(yapping__srv__Duration_Response));
  bool success = yapping__srv__Duration_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
yapping__srv__Duration_Response__destroy(yapping__srv__Duration_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    yapping__srv__Duration_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
yapping__srv__Duration_Response__Sequence__init(yapping__srv__Duration_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  yapping__srv__Duration_Response * data = NULL;

  if (size) {
    data = (yapping__srv__Duration_Response *)allocator.zero_allocate(size, sizeof(yapping__srv__Duration_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = yapping__srv__Duration_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        yapping__srv__Duration_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
yapping__srv__Duration_Response__Sequence__fini(yapping__srv__Duration_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      yapping__srv__Duration_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

yapping__srv__Duration_Response__Sequence *
yapping__srv__Duration_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  yapping__srv__Duration_Response__Sequence * array = (yapping__srv__Duration_Response__Sequence *)allocator.allocate(sizeof(yapping__srv__Duration_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = yapping__srv__Duration_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
yapping__srv__Duration_Response__Sequence__destroy(yapping__srv__Duration_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    yapping__srv__Duration_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
yapping__srv__Duration_Response__Sequence__are_equal(const yapping__srv__Duration_Response__Sequence * lhs, const yapping__srv__Duration_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!yapping__srv__Duration_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
yapping__srv__Duration_Response__Sequence__copy(
  const yapping__srv__Duration_Response__Sequence * input,
  yapping__srv__Duration_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(yapping__srv__Duration_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    yapping__srv__Duration_Response * data =
      (yapping__srv__Duration_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yapping__srv__Duration_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          yapping__srv__Duration_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!yapping__srv__Duration_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
